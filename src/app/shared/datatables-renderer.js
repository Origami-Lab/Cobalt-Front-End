if (typeof (DatatablesRenderer) == 'undefined') var DatatablesRenderer = function () {
        ...
        dRenderer.Renderer.prototype = {
            ...
            buildTabularLineData: function (tblJSONObj, tblProperties, isLastRow = false) {
                var htmlTbl = "";
                var tblClass = tblJSONObj.tblClass;
                var payload = tblJSONObj.payload;
                if (!tblProperties || tblProperties.length == 0) {
                ...
                var printViewTblTDStyles = "padding: 5px 7px;word-wrap: break-word!important;";
                if(isFirstRow){
                    htmlTbl = "<table class='" + tblClass + "' style='border: 1px solid #000;" + printViewTBlStyles + "background-color:white;width:" + tblWidth + "%!important;height:" + tblHeight + "px!important;'><tbody>";
                }
                var rowVAlign = typeof (rowAttrs) == 'undefined' || rowAttrs == null ? "left" : rowAttrs.rowVAlign || "left";
                // the tables contains only one row, no need to do a FOR
                for (var j = 0, rl = payload.length; j < rl; j++) {
                    var tds = payload[j];
                    htmlTbl += "<tr style='vertical-align:" + rowVAlign + "'>";
                    for (var i = 0, tl = tds.length; i < tl; i++) {
                        if (tds[i].indexOf('/r/n') != -1) {
                            cellsWithBr = "";
                            var tdText = tds[i].split('/r/n');
                            ...
                            htmlTbl += "<td " + " style='border: 1px solid #000;" + printViewTblTDStyles + "' >" + cellsWithBr + "<br value='tblBreak'></td>";
                        } else {
                            htmlTbl += "<td " + " style='border: 1px solid #000;" + printViewTblTDStyles + "' >" + tds[i] + "<br value='tblBreak'></td>";
                        }
                    }
                    htmlTbl += "</tr>";
                }
            ...
            getLineHtml: function (code, isLastRow = false) {
                var JSONCode = "";
                var html = "";
                try {
                    JSONCode = JSON.parse(code);
                    tblProperties = JSONCode.tblProperties;
                    html = this.buildTabularLineData(JSONCode, tblProperties, isLastRow);
                } catch (error) {}
                return html;
            }
        };
